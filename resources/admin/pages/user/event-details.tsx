...
export default function PageEventDetails({ event }: { event: EventDetails }) {
    const { event: even, registrations } = event as { event: EventModel, registrations: EventRegistration[] };
                ...
                <div className="@container/main flex flex-1 flex-col gap-2">
                    ...
                    <div className="flex flex-col gap-4 py-4 md:gap-6 md:py-6">
                        ...
                        <div className="flex flex-col gap-2">
                            ...
                            {/* تاريخ بدا  وقت البدا - وقت الانتهاء عنوان اذا وجد او عبر اذا كان افتراضي عبر  */}
                            <div className="flex items-center text-sm text-muted-foreground flex-wrap gap-3 md:gap-5">
                                <div className="flex items-center text-sm text-muted-foreground gap-2">
                                    <Calendar className="size-4" />
                                    <span>
                                        {even.start_at ? format(new Date(even.start_at), "d MMMM yyyy", { locale: ar }) : 'تاريخ غير محدد'}
                                    </span>
                                </div>
                                <div className="flex items-center text-sm text-muted-foreground gap-2">
                                    <Clock className="size-4" />
                                    <span>
                                        {/* وقت البدا - وقت الانتهاء */}
                            ...
                            <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
                                {registrations.map((reg: EventRegistration) => (
                                    <AttendeeCard
                                        key={reg.id}
                                        registration={reg}
                                        onAttendanceToggle={(registrationId, isAttended) => {
                                        }}
                                    />
                                ))}
                            </div>
