<div @class([
    ...
    <div class="p-2 text-center">
        ...
        <h3 class="text-lg text-foreground">{{ $membership->name }}</h3>
        <p class="text-sm text-muted-foreground mt-2 min-h-[40px]">{{ $membership->description }}</p>

        <div class="my-2">
            <div class="text-xl font-bold text-primary flex items-center justify-center gap-1">
                <span>{{ $membership->regular_price }}</span>
            ...
            @if ($membership->is_discounted)
                <div class="text-sm text-muted-foreground line-through">
                    {{ $membership->price }} <x-ui.icon name="riyal" class="inline h-4 w-4" />
                </div>
                <div class="text-xs text-green-600 dark:text-green-400">
                    {{ __('memberships.save_amount', ['amount' => $membership->price - $membership->discounted_price, 'currency' => 'ريال']) }}
                    {{-- حفظ: US$ 537.60 --}}
                    ({{ __('memberships.save_percentage', ['percentage' => $membership->precentage_discount]) }})
                    {{-- حفظ 75% --}}
                </div>
            @endif
            ...
            @if ($membership->is_discounted)
                <div class="mt-1 text-xs text-muted-foreground">
                    {{-- 48 شهرًا مقابل US$ 124.32 (السعر العادي US$ 575.52). يتم التجديد بسعر US$ 10.99/الشهر. --}}
    ...
    <div x-data="{ showAllBenefits: false }" class="p-6 flex flex-col flex-grow bg-muted/30 rounded-t-2xl border-t border-border">
        {{-- المتطلبات --}}
        @if (!empty($membership->requirements))
            <div class="mb-6">
                ...
                <ul class="space-y-2.5 text-sm text-muted-foreground">
                    @foreach ($membership->requirements as $req)
        ...
        @if (!empty($membership->features))
            <div class="mb-6">
                ...
                <ul class="space-y-2.5 text-sm text-foreground">
                    @foreach ($membership->features as $index => $benefit)
                ...
                @if (count($membership->features) > 4)
