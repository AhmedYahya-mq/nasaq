<section class="relative py-20 md:py-24 bg-gradient-to-br from-primary/5 via-background to-background overflow-hidden" aria-labelledby="about-hero-title">
    <div class="md:p-6 p-2 grid grid-cols-1 gap-2.5 md:grid-cols-[1fr_0.8fr] place-items-center">
        ...
        <div class="flex flex-col justify-center not-md:order-2">
            <h1 id="about-hero-title" class="font-bold text-2xl tracking-tight text-primary mb-6 leading-snug">
                {{ __('about.hero.title') }}
            </h1>
            ...
            <!-- النقاط التعريفية -->
            <ul class="space-y-4 rtl:text-right ltr:text-left mb-10 max-w-md mx-auto lg:mx-0" aria-label="Key points">
                <li class="flex items-start gap-3">
                    <span class="w-6 h-6 flex items-center justify-center bg-primary text-white rounded-full shrink-0" aria-hidden="true">✓</span>
        ...
        <div class="relative not-sm:p-2 not-md:order-1">
            <img src="{{ asset('images/about.png') }}"
                 alt="Illustration of community collaboration"
                 class="rounded-2xl shadow-lg object-cover aspect-[6/4]" loading="lazy" />
        </div>
    ...
    <div class="max-w-7xl mx-auto mt-20 px-6 lg:px-12">
        <div class="grid gap-8 md:grid-cols-2 lg:grid-cols-2 justify-center">
            ...
            <div class="p-8 rounded-2xl bg-card shadow-lg border border-border flex flex-col hover:shadow-xl transition">
                <div class="w-16 h-16 flex items-center justify-center rounded-full bg-primary/10 text-primary mb-6" aria-hidden="true">
                ...
                <h2 class="text-2xl font-bold text-primary mb-4">{{ __('about.vision_title') }}</h2>
                <p class="text-muted-foreground mb-6">{{ __('about.vision_text') }}</p>
                @php($visionPoints = __('about.vision_points'))
                @if(is_array($visionPoints))
                    <ul class="space-y-3 text-sm text-muted-foreground" aria-label="Vision points">
                        @foreach($visionPoints as $point)
                            <li class="flex items-start gap-2">
                                <span class="mt-1 w-5 h-5 flex items-center justify-center rounded bg-primary/10 text-primary text-xs" aria-hidden="true">✓</span>
                                <span>{{ $point }}</span>
                            </li>
                        @endforeach
                    </ul>
            ...
            <div class="p-8 rounded-2xl bg-card shadow-lg border border-border flex flex-col hover:shadow-xl transition">
                ...
                <h2 class="text-2xl font-bold text-primary mb-4">{{ __('about.mission_title') }}</h2>
                ...
                @if(is_array($missionPoints))
                    <ul class="space-y-3 text-sm text-muted-foreground" aria-label="Mission points">
