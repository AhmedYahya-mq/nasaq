...
<li class="group/link" {{ $hasSubMenu ? 'x-data=hover x-init=init()' : '' }}>
       ...
       class="flex items-center gap-1 cursor-pointer px-3 py-1 rounded-xl text-foreground
        ...
        @if ($hasSubMenu)
            <x-ui.icon name="chevron-down"
                class="mt-[0.6px] size-4 group-hover/link:rotate-180 duration-200" />
        @endif
    </a>
        ...
        <div class="sub-menu"
            ...
            <div class="grid gap-7 z-10 grid-cols-{{ $menu['gridCols'] ?? 1 }}">
                @foreach ($menu['subMenu'] as $subMenu)
                    <div class="relative cursor-pointer flex items-center gap-x-4 group/menubox">
                        <div class="bg-foreground/10 w-fit p-2 rounded-md
                                    group-hover/menubox:bg-background
                                    group-hover/menubox:text-primary duration-300">
                            @isset($subMenu['icon'])
                                <div class="size-6 border"></div>
                            @endisset
                        </div>
                        <div>
                            <h6 class="font-semibold">{{ __($subMenu['name']) }}</h6>
