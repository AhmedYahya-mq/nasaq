...
init()" x-cloak>
    ...
    <input type="hidden" id="payment_intent_token" value="{{ $intentToken }}">
        ...
        <div class="h-full grid grid-cols-1 xl:grid-cols-[1.1fr_1.2fr] gap-6">
                            ...
                            <div class="flex justify-between text-sm">
                                ...
                                <span class="font-semibold">
                                    ...
                                    @elseif($membershipAction === 'upgrade')
                                        {{ __('payments.Upgrade') }}
                                    @elseif($membershipAction === 'downgrade')
                                        {{ __('payments.Downgrade') }}
                                    @else
            ...
            <div class="w-full flex flex-col gap-6 @container not-md:order-3">
                <div class="rounded-2xl border border-border/70 bg-card/80 backdrop-blur shadow-xl p-6 space-y-4">
                    ...
                    @if ($isMembership)
                        @php($action = $membershipAction ?? 'new')
                        <div class="rounded-xl border border-amber-300/80 bg-amber-50 text-amber-900 px-4 py-3 text-sm flex gap-2 dark:bg-amber-900/20 dark:border-amber-400/70 dark:text-amber-100">
                            ...
                            <div class="space-y-1">
                                <div class="font-medium">{{ __('payments.Membership Notice Title') }}</div>
                                <div>
                                    {{ match ($action) {
                                        'renewal' => __('payments.Membership Notice Renewal'),
                                        'upgrade' => __('payments.Membership Notice Upgrade'),
                                        'downgrade' => __('payments.Membership Notice Downgrade'),
                                        default => __('payments.Membership Notice New'),
                                    } }}
                                </div>
                                <div class="text-xs opacity-90">{{ __('payments.Membership Notice Approval') }}</div>
                            </div>
                        </div>
                    @endif
