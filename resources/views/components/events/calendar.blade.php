@props([
    ...
    'translations' => [
        ...
        'close' => __('events.calendar.close'),
    ]
])

{{--
    - نستدعي مكون Alpine.js المسجل باسم 'eventsCalendar'.
    - نمرر له مصفوفة الفعاليات ($events) ومصفوفة الترجمات ($translations).
    - دالة json_encode تحول مصفوفات PHP إلى كائنات JavaScript.
--}}
<div x-data="eventsCalendar({{ json_encode($events) }}, {{ json_encode($translations) }})"
     x-init="init()"
     class="bg-card p-4 sm:p-6 rounded-2xl border border-border shadow-lg overflow-hidden">
    ...
    <div class="grid grid-cols-7 gap-1 sm:gap-2">
        ...
        <template x-for="date in daysInMonth" :key="date">
            <div
                ...
                @click="openDay(date)"
            >
                <div class="text-xs sm:text-sm flex justify-center sm:justify-start"><span x-text="date"></span></div>
                ...
                <div class="absolute bottom-1.5 left-0 right-0 flex justify-center items-end gap-1">
                    <template x-for="event in eventsForDate(date).slice(0, 3)">
                        <span class="h-1.5 w-1.5 sm:h-2 sm:w-2 rounded-full" :class="{ 'bg-blue-500': event.type === 'Zoom', 'bg-green-500': event.type === 'حضوري', 'bg-yellow-500': event.type === 'ورشة' }"></span>
                    </template>
                </div>
            </div>
        </template>
    ...
    <div x-show="showModal" @keydown.escape.window="showModal = false" x-transition:enter="ease-out duration-300" x-transition:enter-start="opacity-0" x-transition:enter-end="opacity-100" x-transition:leave="ease-in duration-200" x-transition:leave-start="opacity-100" x-transition:leave-end="opacity-0" class="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4" style="display: none;">
        <div @click.away="showModal = false" x-show="showModal" x-transition:enter="ease-out duration-300" x-transition:enter-start="opacity-0 scale-95" x-transition:enter-end="opacity-100 scale-100" x-transition:leave="ease-in duration-200" x-transition:leave-start="opacity-100 scale-100" x-transition:leave-end="opacity-0 scale-95" class="bg-card rounded-2xl shadow-lg w-full max-w-md">
            <div class="p-6 border-b border-border"><h3 class="text-lg font-bold" x-text="modalTitle"></h3></div>
            ...
            <div class="p-4 bg-accent/10 rounded-b-2xl text-center">
                 <button class="text-sm font-semibold text-primary hover:underline" @click="showModal = false" x-text="trans.close"></button>
            </div>
        </div>
    </div>
</div>
