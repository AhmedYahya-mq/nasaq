<article {{ $attributes->merge([
    'class' => 'relative w-full bg-card rounded-2xl shadow-lg hover:shadow-2xl transition-all duration-500 overflow-hidden group cursor-pointer transform hover:-translate-y-1 border border-border'
    ...
    <div class="absolute top-4 right-4 z-20">
        ...
        @endphp
        <span class="inline-flex items-center px-3 py-1.5 rounded-full text-xs font-bold text-white shadow-lg transform rotate-3"
    ...
    <div class="relative z-10 p-6">
        ...
        <section class="mb-6">
            ...
            <div class="flex items-center justify-between mb-2">
                <div class="flex items-center gap-3">
                    ...
                    <span class="text-lg font-bold text-card-foreground" itemprop="attendeeCount">{{ $registered }}</span>
                </div>
                <div class="text-right">
                    ...
                    @else
                        <div class="flex items-center gap-2">
                            ...
                            <span class="text-lg font-bold {{ $remaining <= 10 ? 'text-red-600 dark:text-red-400' : 'text-primary' }}">
                                {{ $remaining }}
                            </span>
                        </div>
                    @endif
        ...
        <section class="mb-4" aria-label="العضويات المسموحة">
            ...
            <button onclick="toggleMemberships(this)"
                ...
                <div class="flex items-center gap-2 flex-1">
                    <div class="w-6 h-6 bg-primary/10 rounded-lg flex items-center justify-center">
                        <svg class="w-3 h-3 text-primary" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z"/>
                        </svg>
                    </div>
                    <span class="text-sm font-medium text-card-foreground">العضويات المسموحة</span>
                    <span class="text-xs bg-primary text-primary-foreground px-2 py-0.5 rounded-full">{{ count($membership_names) }}</span>
                </div>
            ...
            <div id="membership-list" class="membership-list hidden mt-2 pl-8">
                <div class="flex flex-wrap gap-1 max-h-28 scrollbar">
                    ...
                    <span class="inline-flex items-center px-2 py-1 rounded-full text-xs bg-primary/10 text-primary border border-primary/20">
                        {{ $membership }}
                    </span>
                    @endforeach
                </div>
        ...
        <section class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
            <!-- التاريخ -->
            <div class="flex items-center gap-3 p-3 bg-card rounded-xl shadow-sm border border-border" itemprop="startDate" content="{{ $event->start_at->toIso8601String() }}">
            ...
            <div class="flex items-center gap-3 p-3 bg-card rounded-xl shadow-sm border border-border">
                ...
                <div>
                    ...
                    <div class="text-sm font-semibold text-card-foreground">
                        {{ $event->start_at->locale(app()->getLocale())->translatedFormat('h:i A') }}
                    </div>
                </div>
            </div>
            <!-- الموقع -->
            @if ($event->event_type->isPhysical())
            <div class="flex items-center gap-3 p-3 bg-card rounded-xl shadow-sm border border-border md:col-span-2" itemprop="location" itemscope itemtype="http://schema.org/Place">
                <div class="flex-shrink-0 w-10 h-10 bg-primary/10 rounded-lg flex items-center justify-center">
                    <svg class="w-5 h-5 text-primary" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
...
<script>
function toggleMemberships(button) {
